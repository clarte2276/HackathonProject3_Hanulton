...
const getBoardData = (tableName, res) => {
  const query = `SELECT no, title, nickname, content, DATE_FORMAT(created_date, '%Y년 %m월 %d일 %H시 %i분') AS created_date, state, file_data, place FROM ${tableName} ORDER BY created_date DESC`;
